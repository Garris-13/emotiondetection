...
REM ========================================================
REM 5. 检查并安装项目依赖 (海外官方源 + 进度可见版)
REM ========================================================
set "INSTALL_MARKER=%VENV_DIR%\.deps_installed"

if not exist "%INSTALL_MARKER%" (
    echo [5/7] 首次运行或环境缺失，正在安装项目依赖...
    echo 💡 提示: 已去除静默模式，您将看到实时的下载进度！

    echo 正在升级pip...
    "%VENV_PYTHON%" -m pip install --upgrade pip

    echo 正在安装兼容性依赖...
    "%VENV_PYTHON%" -m pip install flask==2.3.3 werkzeug==2.3.7
    "%VENV_PYTHON%" -m pip install flask-cors>=4.0.0 --upgrade

    if exist "requirements.txt" (
        echo.
        echo 🚀 正在从 requirements.txt 安装核心依赖...
        echo ⚠️ 注意: 包含 PyTorch 等超大文件，大约2GB，在当前网络下通常需要 1 到 3 分钟，请耐心等待进度条走完！
        "%VENV_PYTHON%" -m pip install -r requirements.txt --ignore-installed flask flask-cors werkzeug
    )
