...
chcp 65001 >nul
echo ========================================
echo  表情识别系统 - Python 3.13 兼容版启动脚本
echo ========================================
echo.
...
REM 4. 安装项目依赖（修复 Python 3.13 兼容性问题）
echo [5/6] 安装项目依赖...
echo 正在升级pip...
...
REM 强制升级 flask-cors 到兼容 Python 3.13 的版本
echo 安装兼容 Python 3.13 的 flask-cors...
"%VENV_PYTHON%" -m pip install flask-cors>=4.0.0 --upgrade --quiet

REM 安装其他依赖
if exist "requirements.txt" (
    echo 从requirements.txt安装其他依赖...
    REM 跳过已安装的依赖
    "%VENV_PYTHON%" -m pip install -r requirements.txt --quiet --ignore-installed flask flask-cors werkzeug
)
